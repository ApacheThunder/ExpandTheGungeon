...
namespace ExpandTheGungeon.ExpandComponents {
    ...
    public class ExpandGungeoneerMimicBossController : BraveBehaviour {
        ...
        /*public static AIActor SpawnPlayerMimicBoss(RoomHandler CurrentRoom, IntVector2 position, PlayerController sourcePlayer = null, bool IsSecretFloorBoss = true) {

            GameObject aiActorObject = Instantiate(EnemyDatabase.GetOrLoadByGuid("01972dee89fc4404a5c408d50007dad5").gameObject); // bullet_kin
            ...
            Texture2D BossCardBackground = ResourceExtractor.GetTextureFromResource("Textures\\GungeoneerMimicAssets\\MimicInMirror_BossCardBackground.png");
            ...
            Destroy(CachedEnemyActor.aiShooter.gunAttachPoint.gameObject.GetComponent<tk2dSprite>());
            ...
            AIActor aiActorTarget = AIActor.Spawn(CachedEnemyActor, (position + CurrentRoom.area.basePosition), CurrentRoom, false, AIActor.AwakenAnimationType.Awaken, false);

            if (aiActorTarget.GetComponent<ExpandGungeoneerMimicIntroDoer>()) {
                GenericIntroDoer genericIntroDoer = aiActorTarget.gameObject.GetComponent<GenericIntroDoer>();
                FieldInfo field = typeof(GenericIntroDoer).GetField("m_specificIntroDoer", BindingFlags.Instance | BindingFlags.NonPublic);
                field.SetValue(genericIntroDoer, aiActorTarget.GetComponent<ExpandGungeoneerMimicIntroDoer>());
            }

            ExpandUtility.ApplyCustomTexture(aiActorTarget, prebuiltCollection: sourcePlayer.sprite.Collection);
            if (aiActorTarget.aiShooter && sourcePlayer.primaryHand) {
                List<PlayerHandController> m_attachedHands = ReflectionHelpers.ReflectGetField<List<PlayerHandController>>(typeof(AIShooter), "m_attachedHands", aiActorTarget.aiShooter);
                if (m_attachedHands != null && m_attachedHands.Count > 0) {
                    foreach (PlayerHandController hand in m_attachedHands) {
                        ExpandUtility.DuplicateSprite(hand.sprite as tk2dSprite, sourcePlayer.primaryHand.sprite as tk2dSprite);
                    }
                }
            }

            // aiActorTarget.aiAnimator.facingType = AIAnimator.FacingType.Target;
            aiActorTarget.aiAnimator.IdleAnimation = new DirectionalAnimation() {
                Type = DirectionalAnimation.DirectionType.None,
                Prefix = string.Empty,
                AnimNames = new string[0],
                Flipped = new DirectionalAnimation.FlipType[0],
            };
            aiActorTarget.aiAnimator.MoveAnimation = new DirectionalAnimation() {
                Type = DirectionalAnimation.DirectionType.None,
                Prefix = string.Empty,
                AnimNames = new string[0],
                Flipped = new DirectionalAnimation.FlipType[0],
            };
            aiActorTarget.aiAnimator.HitAnimation = new DirectionalAnimation() {
                Type = DirectionalAnimation.DirectionType.None,
                Prefix = "hit",
                AnimNames = new string[0],
                Flipped = new DirectionalAnimation.FlipType[0],
            };
            aiActorTarget.aiAnimator.enabled = false;

            aiActorTarget.spriteAnimator.Stop();
            aiActorTarget.spriteAnimator.DefaultClipId = 0;
            aiActorTarget.spriteAnimator.playAutomatically = false;
                ...
                if (!string.IsNullOrEmpty(clip.name)) {
                    if (clip.name.ToLower() == "dodge") {
                        ...
                        if (clip.frames != null && clip.frames.Length > 0) {
                            if (!sourcePlayer.UseArmorlessAnim) { aiActorTarget.sprite.SetSprite(clip.frames[0].spriteId); }
                        }
            ...
            if (m_AnimationClips.Count > 0) { aiActorTarget.spriteAnimator.Library.clips = m_AnimationClips.ToArray(); }
            
            aiActorTarget.healthHaver.RegenerateCache();

            if (IsSecretFloorBoss) {
                PickupObject rat_key = PickupObjectDatabase.GetById(727);
                if (rat_key) { aiActorTarget.AdditionalSafeItemDrops.Add(rat_key); }
            }

            Destroy(aiActorObject);

            aiActorTarget.behaviorSpeculator.InterruptAndDisable();
            aiActorTarget.behaviorSpeculator.AttackBehaviors.Clear();
            aiActorTarget.behaviorSpeculator.MovementBehaviors.Clear();
            aiActorTarget.behaviorSpeculator.OtherBehaviors.Clear();
            aiActorTarget.behaviorSpeculator.TargetBehaviors.Clear();
            aiActorTarget.behaviorSpeculator.OverrideBehaviors.Clear();
            aiActorTarget.behaviorSpeculator.RefreshBehaviors();
            // aiActorTarget.ClearPath();
            // Destroy(m_AIActor.behaviorSpeculator);
            return aiActorTarget;
        }*/
