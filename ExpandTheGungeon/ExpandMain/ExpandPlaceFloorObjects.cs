...
namespace ExpandTheGungeon.ExpandMain {
    ...
    [HarmonyPatch]
    public class ExpandPlaceFloorObjects : MonoBehaviour {
        
        public static ExpandPlaceFloorObjects Instance {
            get {
                if (!m_Instance) {
                    GameObject m_ExpandFloorObjectPlacer = new GameObject("Expand Floor Placer", new Type[] { typeof(ExpandPlaceFloorObjects) }) { layer = 22 };
                    m_Instance = m_ExpandFloorObjectPlacer.GetComponent<ExpandPlaceFloorObjects>();
                }
                return m_Instance;
            }
        }

        private static ExpandPlaceFloorObjects m_Instance;
        ...
        [HarmonyPatch(typeof(Dungeon), nameof(Dungeon.PlaceWallMimics), typeof(RoomHandler))]
        [HarmonyPrefix]
        public static bool PlaceWallMimicsPatch(Dungeon __instance, RoomHandler debugRoom) {
            if (ExpandTheGungeon.loadStatus != ExpandTheGungeon.LoadStatus.LoadFinished && ExpandTheGungeon.loadStatus != ExpandTheGungeon.LoadStatus.Other) return false;
            Instance.PlaceFloorObjects(__instance, debugRoom);
            DestroyInstance();
            return false;
        }


        private int WallMimicsPlaced;
        private int WallMimicsPerRoom;
        private int numWallMimicsForFloor;
        private int currentFloor;

        public void PlaceFloorObjects(Dungeon dungeon, RoomHandler roomHandler) {
            WallMimicsPlaced = 0;
            WallMimicsPerRoom = 1;
            numWallMimicsForFloor = MetaInjectionData.GetNumWallMimicsForFloor(dungeon.tileIndices.tilesetId);
            ...
            try {
                ...
                ExpandJunkEnemySpawner.Instance.PlaceRandomJunkEnemies(dungeon, roomHandler, isBackRoomsDungeon);
                ExpandJunkEnemySpawner.DestroyInstance();
                ...
                if (ExpandSettings.EnableExpandedGlitchFloors) {
                    ...
                    ExpandPlaceCorruptTiles.Instance.PlaceCorruptTiles(dungeon);
                    ExpandPlaceCorruptTiles.DestroyInstance();
                }

                ExpandFloorDecorator.Instance.PlaceFloorDecoration(dungeon);
            ...
            foreach (RoomHandler room in m_Rooms) {
                try {
                    if (PlayerHasCorruptedJunk) {
                        ...
                        ExpandPlaceCorruptTiles.Instance.PlaceCorruptTiles(dungeon, room, isCorruptedJunkRoom: true);
                        ExpandPlaceCorruptTiles.DestroyInstance();
                    }
                    ExpandPlaceCorruptedEnemies.Instance.PlaceRandomEnemies(dungeon, currentFloor, room);
