...
namespace ExpandTheGungeon.ExpandMain {
    ...
    public class ExpandSynergies {
        ...
        public static void Init() {
            ...
            foreach (AdvancedSynergyEntry entry in m_TempSynergyList) {
                if (!string.IsNullOrEmpty(entry.NameKey)) {
                    string m_NameCode = "#EXSYNERGY_" + entry.NameKey.Replace(" ", "_").Replace("(", "_").Replace(")", string.Empty).ToLower();
                    string m_NameCodeLocalized = entry.NameKey;

                    entry.NameKey = m_NameCode;
